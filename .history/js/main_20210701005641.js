...
function saveHandler(eleID, idSave) {
  getMyEle(eleID).addEventListener("click", function () {
    ...
    var id = this.id;
    var mangTemp = id.split('_');
    viTri = dSNV.timViTriTheoMa(mangTemp[1]);
    ...
    for (let i = 1; i < trSua.length; i++) {
      ...
      dSNV.DanhSachNV[viTri][i] = trSua[i].innerText;
    }
  })
}
...
function hienThiTimNhanVienTheoMa() {
  ...
  if (nhanVien !== undefined) {
    ...
    getMyEle('thetBody').innerHTML = `
  <tr>
  <td class="${nhanVien.maNV}">${nhanVien.maNV}</td>
  <td class="${nhanVien.maNV}">${nhanVien.ho}</td>
  <td class="${nhanVien.maNV}">${nhanVien.ten}</td>
  <td class="${nhanVien.maNV}">
