...
function taoDong(tBody, nv) {
  ...
  var sua = "<button class='btn btn-warning' id='sua_" + nv.maNV + "' data-toggle='modal' data - target='#exampleModal' > Sửa</button > "
  var xoa = "<button class='btn btn-danger' id='xoa_" + nv.maNV + "'>Xóa</button>"
  var capNhat = "<button class='btn btn-warning btnCapNhat' id='capNhat_" + nv.maNV + "' data-toggle='modal' data - target='#exampleModal' > Cap Nhat</button > "
  var td = document.createElement('td');
  td.innerHTML = sua + xoa + capNhat;
  tr.appendChild(td);
}

function xuLyThemNhanVienVaoTable() {
  var tBody = getEle('theTBody');
  console.log(tBody);
  var soNhanVien = congTy.DanhSachNhanVien.length;
  tBody.innerHTML = '';
  for (var i = 0; i < soNhanVien; i++) {
    var nhanVien = congTy.DanhSachNhanVien[i];
